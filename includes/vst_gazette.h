...
#define OFFSET_GAZETTE_KILL_LIST_HUMAN 0x8005FEA4
#define OFFSET_GAZETTE_KILL_LIST_BEAST 0x8005FEA6
#define OFFSET_GAZETTE_KILL_LIST_UNDEAD 0x8005FEA8
#define OFFSET_GAZETTE_KILL_LIST_PHANTOM 0x8005FEAA
#define OFFSET_GAZETTE_KILL_LIST_DRAGON 0x8005FEAC
#define OFFSET_GAZETTE_KILL_LIST_EVIL 0x8005FEAE

#define OFFSET_GAZETTE_WEAPON_USAGE_UNARMED 0x8005FEB4
#define OFFSET_GAZETTE_WEAPON_USAGE_DAGGER 0x8005FEB6
#define OFFSET_GAZETTE_WEAPON_USAGE_SWORD 0x8005FEB8
#define OFFSET_GAZETTE_WEAPON_USAGE_GREATSWORD 0x8005FEBA
#define OFFSET_GAZETTE_WEAPON_USAGE_AXEMACE 0x8005FEBC
#define OFFSET_GAZETTE_WEAPON_USAGE_GREATAXE 0x8005FEBE
#define OFFSET_GAZETTE_WEAPON_USAGE_STAFF 0x8005FEC0
#define OFFSET_GAZETTE_WEAPON_USAGE_HEAVYMACE 0x8005FEC2
#define OFFSET_GAZETTE_WEAPON_USAGE_POLEARM 0x8005FEC4
#define OFFSET_GAZETTE_WEAPON_USAGE_CROSSBOW 0x8005FEC6

#define OFFSET_GAZETTE_CHEST_COUNT_CURRENT 0x8005FF38
#define OFFSET_GAZETTE_MAP_COUNT_CURRENT 0x8010A46A

#define OFFSET_GAZETTE_CHEST_COUNT_FLAGS 0x80061958
#define OFFSET_GAZETTE_MAP_COUNT_FLAGS 0x8005ffd8

#define OFFSET_GAZETTE_MAP_COUNT_CHECK_FLAGS 0x800e8508
...
PrintKillList()
{
  ...
  WriteToBackBuffer();
  sprintf_s(szBuffer, _countof(szBuffer), "Beast:   %5i\n", KillListCur.Beast);
  WriteToBackBuffer();
  sprintf_s(szBuffer, _countof(szBuffer), "Undead:  %5i\n", KillListCur.Undead);
  WriteToBackBuffer();
  sprintf_s(
      szBuffer, _countof(szBuffer), "Phantom: %5i\n", KillListCur.Phantom);
  WriteToBackBuffer();
  sprintf_s(szBuffer, _countof(szBuffer), "Dragon:  %5i\n", KillListCur.Dragon);
  WriteToBackBuffer();
  sprintf_s(szBuffer, _countof(szBuffer), "Evil:    %5i\n", KillListCur.Evil);
  WriteToBackBuffer();
}
...
WriteKillList()
{
  FILE *fpKillList = fopen("game_data/score/kill_list.txt", "w");

  fprintf(fpKillList, "== KILL LIST ==\n\n");
  fprintf(fpKillList, "Human:   %5i\n", KillListCur.Human);
  fprintf(fpKillList, "Beast:   %5i\n", KillListCur.Beast);
  fprintf(fpKillList, "Undead:  %5i\n", KillListCur.Undead);
  fprintf(fpKillList, "Phantom: %5i\n", KillListCur.Phantom);
  fprintf(fpKillList, "Dragon:  %5i\n", KillListCur.Dragon);
  fprintf(fpKillList, "Evil:    %5i\n", KillListCur.Evil);
...
PrintWeaponUsage()
{
  ...
  WriteToBackBuffer();
  sprintf_s(
      szBuffer, _countof(szBuffer), "Sword:       %5i\n", WeaponUsageCur.Sword);
  WriteToBackBuffer();
  sprintf_s(szBuffer, _countof(szBuffer), "Great Sword: %5i\n",
      WeaponUsageCur.GreatSword);
  WriteToBackBuffer();
  sprintf_s(szBuffer, _countof(szBuffer), "Axe/Mace:    %5i\n",
      WeaponUsageCur.AxeMace);
  WriteToBackBuffer();
  sprintf_s(szBuffer, _countof(szBuffer), "Great Axe:   %5i\n",
      WeaponUsageCur.GreatAxe);
  WriteToBackBuffer();
  sprintf_s(
      szBuffer, _countof(szBuffer), "Staff:       %5i\n", WeaponUsageCur.Staff);
  WriteToBackBuffer();
  sprintf_s(szBuffer, _countof(szBuffer), "Heavy Mace:  %5i\n",
      WeaponUsageCur.HeavyMace);
  WriteToBackBuffer();
  sprintf_s(szBuffer, _countof(szBuffer), "Polearm:     %5i\n",
      WeaponUsageCur.Polearm);
  WriteToBackBuffer();
  sprintf_s(szBuffer, _countof(szBuffer), "Crossbow:    %5i\n",
      WeaponUsageCur.Crossbow);
  WriteToBackBuffer();
  sprintf_s(szBuffer, _countof(szBuffer), "Unarmed:     %5i\n",
...
WriteWeaponUsage()
{
  FILE *fpWeaponUsage = fopen("game_data/score/weapon_usage.txt", "w");

  fprintf(fpWeaponUsage, "== WEAPON USAGE ==\n\n");
  fprintf(fpWeaponUsage, "Dagger:      %5i\n", WeaponUsageCur.Dagger);
  fprintf(fpWeaponUsage, "Sword:       %5i\n", WeaponUsageCur.Sword);
  fprintf(fpWeaponUsage, "Great Sword: %5i\n", WeaponUsageCur.GreatSword);
  fprintf(fpWeaponUsage, "Axe/Mace:    %5i\n", WeaponUsageCur.AxeMace);
  fprintf(fpWeaponUsage, "Great Axe:   %5i\n", WeaponUsageCur.GreatAxe);
  fprintf(fpWeaponUsage, "Staff:       %5i\n", WeaponUsageCur.Staff);
  fprintf(fpWeaponUsage, "Heavy Mace:  %5i\n", WeaponUsageCur.HeavyMace);
  fprintf(fpWeaponUsage, "Polearm:     %5i\n", WeaponUsageCur.Polearm);
  fprintf(fpWeaponUsage, "Crossbow:    %5i\n", WeaponUsageCur.Crossbow);
  fprintf(fpWeaponUsage, "Unarmed:     %5i\n", WeaponUsageCur.Unarmed);
    ...
    chest_flag_check ChestFlagChecklist[CHEST_COUNT_MAX])
{
  ...
  FILE *fpChestMissing = fopen("game_data/map/chest_missing.txt", "w");

  if (ChestCount == CHEST_COUNT_MAX)
  {
    fprintf(fpChestMissing, "You've got them all. Nice!\n");
  ...
  {
    fprintf(fpChestMissing, "Missing Treasure Chests\n\n");
...
WriteMapFlagsDiffs()
{
  FILE *fpMapFlagsDiff = fopen("debug/map_flag_diff.txt", "a");
  ...
  for (int i = 0; i < MAP_COUNT_FLAGS; ++i)
  {
    ...
    if (FlagPrev != FlagCur)
    {
      ...
      for (int j = 0; j < 8; ++j)
      {
        ...
        if (BitMask & FlagDiff)
        {
          fprintf(fpMapFlagsDiff, "%i,%i,", i, j);
          fprintf(fpMapFlagsDiff, "%s,%s,%i,%i\n", nameZone, nameMap,
...
{
  FILE *fpChestFlagsDiff = fopen("debug/chest_flag_diff.txt", "a");
  ...
  for (int i = 0; i < CHEST_COUNT_FLAGS; ++i)
  {
    ...
    if (FlagPrev != FlagCur)
    {
      fprintf(fpChestFlagsDiff, "%i,", i);
      fprintf(fpChestFlagsDiff, "%s,%s,%i,%i\n", nameZone, nameMap,
          LocationCur.ZoneNumber, LocationCur.MapNumber);
    }
  }
...
GenerateMapCheckFlagList()
{
  ...
  FILE *fpCheckFlags = fopen("debug/map/check_flags.txt", "w");
    ...
    map_flag_mem *MapFlags, map_flag_check MapFlagChecklist[MAP_COUNT_MAX])
{
  ...
  FILE *fpMapMissing = fopen("game_data/map/room_missing.txt", "w");

  if (MapCount == MAP_COUNT_MAX)
  {
    fprintf(fpMapMissing, "You've been everywhere. You've seen everything.\n");
  ...
  {
    fprintf(fpMapMissing, "Missing Locations\n\n");
...
WriteChestCount(u8 *ChestCount)
{
  ...
  FILE *fpChestCount = fopen("game_data/map/chest_count.txt", "w");

  fprintf(fpChestCount, "== CHEST COUNT ==\n\n");
...
WriteMapCount(u16 *MapCount)
{
  ...
  FILE *fpMapCount = fopen("game_data/map/room_count.txt", "w");

  fprintf(fpMapCount, "== ROOM COUNT ==\n\n");
